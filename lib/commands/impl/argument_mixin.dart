...
mixin ArgsMixin {
  final List<String> _args = HpCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(HpCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return HpCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < HpCli.arguments.length) {
      index++;
      return HpCli.arguments[index];
    } else {
